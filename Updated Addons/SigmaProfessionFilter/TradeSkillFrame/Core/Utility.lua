...
function SPF2.TradeSkillFrame_PostUpdate()
	...
	if (LeaPlusDB and LeaPlusDB["EnhanceProfessions"] == "On" and TradeSkillSkill28) then
		if SPF2.Headers and #SPF2.Headers == 0 and SPF2.FIRST then
			TradeSkillSkill1:SetPoint("TOPLEFT", TradeSkillFrame, "TOPLEFT", 22, -81);
			if SPF2.Data and #SPF2.Data > 27  then
				TradeSkillSkill28:Show();
			end
		else
			TradeSkillSkill28:Hide();
			TradeSkillSkill1:SetPoint("TOPLEFT", TradeSkillFrame, "TOPLEFT", 22, -96);
		end
	end
	
	local currentName = GetTradeSkillName();
	if SPF2.TradeSkillName ~= currentName then
		local isProfessionChange = (SPF2.TradeSkillName ~= nil);
		SPF2.TradeSkillName = currentName;
		if isProfessionChange then
			SPF2.FullUpdate();
		end
	end
