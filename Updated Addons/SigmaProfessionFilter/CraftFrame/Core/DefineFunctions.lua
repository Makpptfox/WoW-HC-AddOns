	...
	if not SPF1.FILTERED then
		...
		for i=1, SPF1.baseGetNumCrafts() do
			...
			if craftType == "header" then
				...
				SPF1.OriginalHeaders[i] =
				(ByType["header"][headerIndex] and ByType["header"][headerIndex].name)
				or "";
		...
		if SPF1:SavedData()["Unlearned"] then
			...
			for spellID,spellData in pairs(SPF1.GetRecipeInfo() or {}) do
				...
				-- skip invalid client spells
				if craftName and not SPF1.Recipes[craftName] and SPF1.Starred:Filter(craftName) then
					-- IMPLEMENT CHECKS LATER
					local leftGroupID = SPF1.LeftMenu:FilterSpell(spellID, LeftSelection) or 0;
					local rightGroupID = SPF1.RightMenu:FilterSpell(spellID, RightSelection) or 0;
