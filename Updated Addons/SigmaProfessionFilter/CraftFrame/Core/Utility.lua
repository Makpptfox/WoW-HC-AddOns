...
function SPF1.ClearNewFeatures()
	if CraftFrameFilterDropDown then CraftFrameFilterDropDown:Hide() end
	if CraftFrameAvailableFilterCheckButton then
		CraftFrameAvailableFilterCheckButton:SetChecked(false)
		CraftFrameAvailableFilterCheckButton:Hide()
	end
	if CraftOnlyShowMakeable then CraftOnlyShowMakeable(false) end
end
...
function SPF1.FullUpdate(keepCollapsed)
	...
	--LTP handles that correctly by itself now.
