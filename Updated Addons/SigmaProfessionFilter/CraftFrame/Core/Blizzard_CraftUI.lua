...
function SPF1.CraftFrame_Update()
	...
	-- ScrollFrame update
	FauxScrollFrame_Update(CraftListScrollFrame, numCrafts, 27, CRAFT_SKILL_HEIGHT, nil, nil, nil, CraftHighlightFrame, 293, 316 );
    -- ... (for i=1, 27, 1 do)
	...
	for i=1, CRAFTS_DISPLAYED, 1 do
		...
		if ( craftIndex <= numCrafts ) then
			...
			if ( craftType == "header" ) then
				...
				-- restore alpha hidden by non-headers
				if craftButton:GetNormalTexture() then
					craftButton:GetNormalTexture():SetAlpha(1)
				end
				...
				-- force hide xml template default texture
				if craftButton:GetNormalTexture() then
					craftButton:GetNormalTexture():SetAlpha(0)
				end
	...
	if CraftFramePointsLabel and (not (LeaPlusDB == nil) and LeaPlusDB["EnhanceProfessions"] == "On") then
		CraftFramePointsLabel:ClearAllPoints()
		-- Placing the training points text a bit higher to not stick to the checkboxes
		CraftFramePointsLabel:SetPoint("TOPLEFT", CraftFrame, "TOPLEFT", 140, -40)
		-- Bigger font to be more obvious
		local f1, _, fl1 = CraftFramePointsLabel:GetFont()
		CraftFramePointsLabel:SetFont(f1, 13, fl1)
		
		if CraftFramePointsText then
			local f2, _, fl2 = CraftFramePointsText:GetFont()
			CraftFramePointsText:SetFont(f2, 13, fl2)
		end
	end
