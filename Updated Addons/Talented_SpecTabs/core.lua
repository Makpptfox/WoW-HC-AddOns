...
local GetNumGroups = GetNumTalentGroups or GetNumSpecGroups or function() return 1 end
local SetActiveGroup = SetActiveTalentGroup or SetActiveSpecGroup
...
local function UpdateSpecInfo(info)
    ...
    for i = 1, tabs do
        ...
        if points < min then min = points end
        if points > max then
            imax, max = i, points
        end
        total = total + points
...
local function TabFrame_OnClick(self, button)
    ...
    if button == "RightButton" then
        if not info.pet and not InCombatLockdown() then
            SetActiveGroup(info.talentGroup)
